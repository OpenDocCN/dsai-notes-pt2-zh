...
所以，这不会改变，但K会随着时间的推移增长。当我们迈出越来越多的步伐时，分子的常数与分母的增长速度相比，分母的增长速度非常快。所以我们可以说，梯度下降是以1/k的速率收敛的。这就是正在发生的变化。当k趋于无穷大时，收敛速度被认为是非常慢的。我们稍后会看到一张图，说明这种所谓的次线性率。嗯，梯度下降在实践中可能做得更好，尤其是在某些特定的例子中，因为f函数具有额外的性质。我再多说一句，有一种方法可以从误差的角度来考虑这个比率。如果xk减去x星的f，这就像是在衡量我们的次优程度，我们距离f的最小值有多远。所以如果我们想让误差小于或等于ε，对于这种类型的方法，我们将不得不采取与Epsilon步骤成正比的数量。那是什么意思呢？假设我们希望Epsilon是100中的一个，我们就需要进行约100步的操作，甚至可能是1000步。

我们需要一个与步数成正比的东西。换句话说，对于这种情况，如果我没弄错的话，如果我们想提高精度，增加一个额外的小数点，我们需要让损失从1.5降到1.4，就必须跑十倍的次数。顺便提一句，这是一个固定的步长。但同样的速率也适用于回溯线搜索。所以，即使感觉速度快得多，但本质上速率是相同的，区别在于定理中的步长。我们需要知道Lipschitz常数，而回溯线搜索不需要了解这些。好的，这是梯度下降的一个弱版本。从某种意义上讲，我们并没有对f假设太多，而更多地假设f具有某些特性。接下来是强凸性，像岭回归中，我们有这种经验风险，并且我们加上了w平方的范数作为二次项。这样，当你处理一个凸函数时，加入一个严格的二次项后，它会变得非常凸。这里有一个条件，但强凸的定义条件，嗯，哦，我们稍后再讨论这个问题。等我们定义凸性时，这将更有意义，f是右边两项的组合，忘记第三项，f是凸的。

只有当这个不等式完全成立时，强凸性才要求不等式成立，即使我们在右手边加上一个额外的平方项。当我们远离x时，f的增长速度必须比f更快，或者在强凸的情况下增长得更快。我看到有人点头。这是令人鼓舞的。我不确定，我不认为这会太强烈，因为它具备最小的二阶导数，可能黑森矩阵的最小特征值会有所帮助。我来告诉你，那里有一些东西。好的，如果f是强凸的，我们有一个更强大的定理。结合了之前的Lipschitz条件，并加入了强凸性。然后，无论是使用固定步长还是回溯线搜索进行梯度下降，都可以满足这个新的边界。我们会看到它的表现会更好。所以这里有一个常数c，位于0和1之间，步数是c的指数，所以如果你有一个位于零到一之间的数字，我们把它提升到指数。

当k和k不断增大时，c会迅速衰减，指数下降速度很快。所以它会以指数级的速度快速归零，剩余时间和其他因素会随着k的增加保持不变，因此这将加速收敛。因此，如果我们在强凸优化问题上做梯度下降，其他条件下，收敛速度会快得多，这就是岭回归的情况，这也是为什么我们看到梯度下降明显优于随机梯度下降的原因。我将会进一步描述这一点。好吧，另一种方法，另一种写法就是指数衰减，特别是在具有强凸性的问题中。为了达到Epsilon精度，我们只需要进行1/Epsilon次迭代，所以在我们需要超过Epsilon次迭代之前，在强凸性内，我们只需要在Epsilon上进行一次记录，这将带来显著的效果。好的，这里有一个更像教科书情节的示例，黑线是全梯度下降，其他三条线是随机梯度下降，批量大小为110和100，它们基本相似，但全梯度下降明显更快，y轴为对数刻度。

所以你会看到，在对数尺度上是线性衰减，这就是为什么这种类型的下降通常被称为线性收敛。所以下次你听到线性收敛时，意味着当你在对数刻度上绘制误差时，表现为一条直线。其他这些则被称为亚线性收敛，亚线性性能是指在对数尺度上看起来比线性衰减慢得多。我意识到这一点了，所以我不得不指出，随机梯度下降的收敛速度也是如此，它表现为亚线性收敛。我相信它的速率甚至比我们的一些方法更慢，我相信它接近于Epsilon的平方根，但我会再帮你确认一下。好的，最后一张幻灯片基本上提出了一个问题，我们关心这些截然不同的收敛率吗？你知道，如果新元这么差，我们为什么还要用它？答案是，对我们的大多数目的来说，这真的无关紧要。这里有一张幻灯片做了说明，所以这里有两种方法，SGD和一种叫做创的算法，我们只认为它是有效的，tron是一种二阶方法，速度非常快。

x轴表示优化精度，因此这是在你的训练集上，表示你有你的损失，训练损失与理想训练损失之间的差距。当你运行优化算法时，从左边开始，训练损失是第一点，它做了更多的工作，使其更接近零。在一次精度测试中，x轴表示时间，y轴是训练时间。为了达到小数点后五位的精度，随机梯度下降要比常规方法快得多，但当我们需要达到小数点后八位，甚至九位精度时，随机梯度下降会变得非常慢，接近最佳值时，它会变得很慢。所以，如果我们需要非常高的精度来投射屏幕，那就不适用了。不过在这张图上是预期风险的图表，你可以想象他们有一个巨大的测试集，基于这个大测试集，他们训练并评估预测算法的表现。预期风险接近零时，他们看到预期风险快速下降，然后稳定下来，不再有显著改善。当我们把优化误差精度改进到第六、第七、第八、甚至第九位时，这里要讨论的是你为额外时间付出的代价。

你正在运行你的算法，回到交易的话题，我想我们在第二课讲过，优化误差与估计误差和逼近误差之间的关系。当我们运行时，时间越来越长，优化误差在减少，但不清楚我们的估计误差是否相应地得到改善。这是对这种情况的一个很好的描述，尽管有点高级，但你可以将其理解为大规模学习中的权衡。实际上我们看到的情况是，随着迭代次数增加，运行时间逐渐变长，这些应该是正常化的，所以它们就像一个时代一样。如果你使用随机梯度下降，结果可能会相似。所以，假设我们已经进入了PAC理论中的比较部分，这时候做比较没有多大意义，你觉得这会是什么样的呢？即使是最糟糕的情况，它实际上也是每一步都算作一次迭代。它会压缩发生的事情，通过常数因子来调整新币的大小，这是训练集的大小。是的，你觉得这样做更好吗？我不确定，我不认为效果会太好，因为你在改变一些不变的因素。
...
然后它以g为界，所以，即使它是不可微的，到目前为止，这些扭结，它不能很快地跳起来，如果利普希茨常数是斜率，这是最大的角度，你不能，所有的权利，所以如果我们有这个条件，我们有一个台阶大小。根据罗宾斯·门罗的条件而减小，记住一个关于光学平方稳定性和稳定性的东西，例如：We don’t我们的机器运转得很好，1/t的平方根很好，所以如果我们的步长以这种方式减小。那么一个人可以证明不比我们在这里用太多的功，所以我们没有时间，但我们离实际的收敛证明并不太远，这有点酷，你可以证明这种次梯度方法确实出现了，不会改变SI，有很多翻转，你担心这个吗，不应该是否定的。我是说这个人是积极的，是呀，对于合适的T大小，是阴性的，是啊，是啊，是呀，所以说，我想我一直走到最后一行，我想说的是下一个，的形成，不一定是右手边，我们一定找不到，右手边应该总是比左手边大。
...
我们来谈谈，让我们举一个例子，假设我们想算出我们得到的字符串，是电子邮件地址，所以输入，字符的字符串序列，那么我们如何从中制作功能，所以我们可以思考这个问题然后手工制作一些功能，例如，说，你知道的。绳子的长度大于十吗，是或否，是的会是一个，一个不就是零，这将是一个功能，嗯，字符串中的字母字符的分数是多少，85个特征只是编造的，它是否包含应用程序标志，是或不是一个或零。它这样做是一个，它以coit结束吗，对于风暴特性这样做，这是未来工具的一种方法，一个，嗯，但这有点临时的，我想有一个，解决机器学习问题的一种策略，试图让你的电脑为你做很多工作，尽量减少自己的。我们能用更系统的东西来取代这种类型的东西吗，这就进入了特性模板的概念，所以吸引的是一组功能，它们都是以类似的方式计算的，通常你有一段代码来计算所有功能的值，同时，特征模板的长度可以大于空白。
...
这是一个非常，呃，使用的非常高效的数据结构，你可以在GPU上运行，嗯，但是如果您的大多数特性值都为零，它浪费了很多空间，所以有一种叫做稀疏表示的替代表示，在这里，您只存储功能名称和值对。但你漏掉了所有的零，因此，如果特征值为零，你只要把它放在你的存储空间里，所以如果你在Python中，你有字典或哈希地图，在Java什么的，所以它是一个，它是从特性的名称到值的查找。所以我们可以使用数组进行密集和映射，Python中的dicts表示稀疏，所以说，当然啦，当你使用字典时，你有一些开销，因为你必须查找条目，你不能直接访问它们，你不能抹去，但是在很多情况下。尤其是像自然语言处理，我们有稀疏的特征，它是，值得交换，到目前为止，对这些事情有什么问题吗？很容易走对了，所以现在，我们讨论的是如何制作功能的几个问题，这解释了我们到目前为止至少局限于模型的事实。

线性模型，你想在现实世界中使用的许多函数都不是线性的，但是如果你被限制使用线性摩擦函数，你能做什么，你要做的就是把非线性区域放入特征中，所以我们将讨论非线性编码的各种方法，它是捕捉某些类型非线性的特征。所以我们想做一个预测，你想如何给某人的健康评分，所以这里的一般哲学是你想提取每一个可能花费你开发的特性，另一件事是你要尽可能少去想它，并且非常通用，所以你可以用很多不同的东西来进行医学诊断，不同的测量。高度，重量，温度，嗯，所以有三种类型的非线性，我们将讨论非单调性，饱和度与特征间的相互作用，我们将打破，所以让我们从一个非常简单的特征图开始，所以我们要把一个人，这是一个非常抽象的空间，输入空间人。所以我们取一个x，我们把这个人x映射到两个数字，一个偏见，我想有一个偏差和x的温度，所以人的体温，所以我想预测一个分数，其中很大的数字是健康的，在少数人中算不了什么，我们的假设空间。
...
所以所有这些参数，你有点，那个效果，嗯，桶越小就越有价值，凸性是模型的性质，所以是的，这不仅仅是模型的属性，财产，它是损失函数、模型和正则化，所有的权利，所以又来了一个，所以我们做了，我们所做的饱和度。姓互动，所以说，身高和体重，一种经典的例子，嗯，所以它是，重要的是重量相对于身高，那么你怎么得到，得到线性模型，嗯，所以再一次，所以我们做了一些身高和体重之间的相互作用，这样你就可以谷歌了，这是老派的。你谷歌就像理想的，远离高度，知道，有一个家伙作为公式，所以它是两公斤英寸，这只是一个奇怪的，不是吗？我们可以看看预测和实际之间的平方dv问题，我们能做什么呢，所以如果你真的擅长数学。可以把这个表达式展开，嗯，像这样，你看到了你需要的所有条款

所以与其做一个公式，你可以把高度、体重的平方、广场、重量的平方，以及交叉项，这些可以适应这个决策边界，这种功能，或者其他可能更好的东西，这更符合我们的目标，是啊，是啊，所以x的h除以w x。所以这是一个很好的问题，所以说，好的，我会怎么做，是啊，是啊，我喜欢那样，是啊，是啊，那就会，那就会，就会，增加模型的x，他们做的是原木做的单生理学，然后日志之间的差值是比率的日志，作为比率的变换。可能不是这样，所以如果你想要实际的比例，你得把它分开放进去，但夺走生命是一个很好的技巧来获得比例，至少在那里的比例变换，是啊，是啊，你知道的，因为你看到了方程，我想问题，嗯，我想我在争论什么。就是你不必，我是说，你不必知道这就是你想要的，你只是说我想做一个更有表现力的模型，我怎样才能让这个模型更有表现力，你可以集思广益，比如，哦，也许我可以表达这一点，然后这个怎么样。
...
二次特征表示的二次表示的维数是多少，大致是d平方，所以它大致是d的平方，因此这两个之间的内积需要的时间，就像d的平方，但结果是内积，有一个聪明的方法可以写它，结果你只需要取原来d值的内积`wx`，然后把它平方。把它们加在一起，这相当于四个二次特征的正则乘积，这有点像数学技巧，好的地方是，然后你可以在大小为d的特征向量上计算这个内积，与d成正比的时间平方，而不是d，所以核加法的一般流程是，我最后一张幻灯片。因此我们有一个经过角化的机器学习算法，所以所有的x都只以内积的形式出现，我们能做的，我上次提到过，我们可以把这个欧几里得内积，换上新的内饰，比如说，表示二次特征内积的。现在我们在这个新的特征空间中有效地运行算法，所以它可以像二次特征表示一样，然后在这种情况下，算法依赖于这个铬矩阵，现在是n乘n而不是d乘d。
